...
int main(int argc, char *argv[]) {
    ...
    int ok = QMessageBox::question(home, "Hello", "Welcome to Tinder for Researchers, let's start",
                                   QMessageBox::Ok); // Display a message box

    if (ok ==
        QMessageBox::Ok) // Check that the button has been clicked and manage the different cases
    {
        int answer = QMessageBox::question(
            home, "Log in", "Have you an account?",
            QMessageBox::Yes | QMessageBox::No); // Open a message box that ask if the user want to
                                                 // log in or sign in
        if (answer == QMessageBox::Yes)          // First case : it is a pre-existent user
        {
            ...
            if (ok && !id.isEmpty()) {
                Author author(
                    id.QString::toStdString()); // it does not work, why ? RISK OF ERROR ALSO

                info.update_from_server(client.getLikes(author));
                // display the app
                home->close();
                window_main main(info, author, client);
                main.main->move(QApplication::desktop()->screen()->rect().center() -
                                main.main->rect().center()); // center it
                main.main->show();
            }
        ...
        else // Second case : it is a new user
        {
            QMessageBox::information(
                home, "Sign in",
                "We are going to ask you a few questions about your interests, please answer "
                "carefully");    // Display an information box
                    ...
                    if (t_phy or t_math or t_cs or t_bio or t_fin or t_stat or t_elec or t_econ) {
                        ...
                        if (t_phy) {
                            ...
                            while (windows_precise.scroll_phy->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_phy, SIGNAL(clicked()),
                                                 windows_precise.scroll_phy, SLOT(hide()));
                        ...
                        if (t_math and not t_phy) {
                            ...
                            while (windows_precise.scroll_math->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_math, SIGNAL(clicked()),
                                                 windows_precise.scroll_math, SLOT(hide()));
                        ...
                        if (t_cs and not t_phy and not t_math) {
                            ...
                            while (windows_precise.scroll_cs->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_cs, SIGNAL(clicked()),
                                                 windows_precise.scroll_cs, SLOT(hide()));
                        ...
                        if (t_bio and not t_phy and not t_math and not t_cs) {
                            ...
                            while (windows_precise.scroll_bio->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_bio, SIGNAL(clicked()),
                                                 windows_precise.scroll_bio, SLOT(hide()));
                        ...
                        if (t_fin and not t_phy and not t_math and not t_cs and not t_bio) {
                            ...
                            while (windows_precise.scroll_fin->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_fin, SIGNAL(clicked()),
                                                 windows_precise.scroll_fin, SLOT(hide()));
                        ...
                        if (t_stat and not t_phy and not t_math and not t_cs and not t_bio and
                            ...
                            while (windows_precise.scroll_stat->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_stat, SIGNAL(clicked()),
                                                 windows_precise.scroll_stat, SLOT(hide()));
                        ...
                        if (t_elec and not t_phy and not t_math and not t_cs and not t_bio and
                            ...
                            while (windows_precise.scroll_eess->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_eess, SIGNAL(clicked()),
                                                 windows_precise.scroll_eess, SLOT(hide()));
                        ...
                        if (t_econ and not t_phy and not t_math and not t_cs and not t_bio and
                            ...
                            while (windows_precise.scroll_econ->isHidden() == false) {
                                delay_1();
                                QObject::connect(windows_precise.val_econ, SIGNAL(clicked()),
                                                 windows_precise.scroll_econ, SLOT(hide()));
