...
genGraph() { // generate a graph: a node has a specific signature characterized by 3 integers
...
genInput(std::vector<Eigen::VectorXd>
             &graph) { // generate the input for the clustering ie for each node create a vector of
                       // neighbors (two nodes are neighbors if their similarity is one)
...
int main() {
    ...
    std::cout << "the number of clusters is" << listlabel.size()
              << std::endl; // to know how many clusters we have created
    return 0;
}