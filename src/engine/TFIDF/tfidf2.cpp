...
void TFIDF::update(int threshold){
    ...
    for (std::vector<std::pair<std::string,std::string>>::iterator it=summaries.begin();it!=summaries.end();it++){
            
            if (papers.find(it->first)==papers.end()) {  //replace first by id
              papers.insert(it->first);
              std::vector<std::string> parsed_abstract= textParse(it->second); //replace second by summary
