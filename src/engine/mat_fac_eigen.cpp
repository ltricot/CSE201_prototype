  ...
  public:
    ...
    mat_factorization(Eigen::MatrixXd R, int K, double a, double b, int num_iter) {
        ...
        P = (Eigen::MatrixXd::Random(users, lat_feat) +
             Eigen::MatrixXd::Constant(users, lat_feat, 1.)) *
            0.5;
        Q = (Eigen::MatrixXd::Random(lat_feat, brands) +
             Eigen::MatrixXd::Constant(lat_feat, brands, 1.)) *
            0.5;
    }
    ...
    void kullback_leibler() {
        ...
        Q = Q.cwiseProduct(
            (P.transpose() * (matrix.cwiseQuotient(P * Q))).cwiseQuotient(P.transpose() * one));
        P = P.cwiseProduct(
            ((matrix.cwiseQuotient(P * Q) * Q.transpose())).cwiseQuotient(one * Q.transpose()));
    }
    ...
    void itakura_saito() {
        ...
        P = P.cwiseProduct(((matrix.cwiseQuotient(PQ_square)) * Q.transpose())
                               .cwiseQuotient((one.cwiseQuotient(P * Q) * Q.transpose())));
    }
...
int main() {
    ...
    mat << 5, 0, 0, 3, 4, 0, 0, 1, 1, 0, 0, 5, 1, 0, 0, 4, 0, 1, 0, 4;
