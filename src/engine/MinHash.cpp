...
{   //creates random coefficients for the hash functions (rank = nb of hash functions)
    ...
    while (k > 0){
        ...
        if (std::find(hashes_a.begin(), hashes_a.end(), a) == hashes_a.end()){
            hashes_a.push_back(a); //we add a to our list of coefficients if it is not already in it
            k = k-1;
        };
    };
...
void MinHash::update(Edge edge)
{
    ...
    auto tool = users2articles.insert( std::pair<std::string, std::vector<std::string> > (edge.author.name, vect));
    //if the author already is in the map keys (the author was seen before)
    if (tool.second == false){
        if (std::find(users2articles[edge.author.name].begin(), users2articles[edge.author.name].end(), edge.paper.id) == users2articles[edge.author.name].end()){
