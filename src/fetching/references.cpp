...
template <typename outstream> void extractText(stringstream &istr, outstream &ostr) {
 ...
 * Input : the id of the pdf we want to convert. We have to check whether we've already converted it
 * with the Bulk method. so has to have access to the attributes of the BulkDownloader class ?
 */
PDFConverter::PDFConverter(std::string id) {
    // curl_global_init(CURL_GLOBAL_ALL);  i'm not sure if we should do it here, we only need to do
    // it once per program
    ...
    curl_easy_setopt(handle, CURLOPT_VERBOSE,
                     1L); // very useful for libcurl and/or protocol debugging and understanding
    curl_easy_setopt(handle, CURLOPT_NOPROGRESS,
                     1L); // tells the library to shut off the progress meter completely for
                          // requests done with this handle.
...
PDFConverter::PDFConverter(std::stringstream *pdf) { pdfBuffer << pdf->str(); }
...
PDFConverter::PDFConverter(PDF pdf) {
    ...
    std::vector<unsigned char> pdfBuffer(std::istreambuf_iterator<char>(input),
...
void Papers::initialize() { this->mhandle = curl_multi_init(); }

void Papers::perform() { curl_multi_perform(this->mhandle, stillRunning); }

void Papers::cleanup() { curl_multi_cleanup(this->mhandle); }
...
Papers::Papers(std::vector<std::string> ids) : ids(ids) {
    ...
    // kinda lost with the select thingy to know when a tranfer is done and when we can get the data
    // (i.e. call getText right?)
...
void writeFile(std::map<std::string, std::vector<std::string>> data, std::string filename) {
    ...
    for (std::map<std::string, std::vector<std::string>>::iterator pair = data.begin();
         pair != data.end(); pair++) {
        fs << pair->first << endl;

        for (std::vector<std::string>::iterator ref = pair->second.begin();
             ref != pair->second.end(); ref++) {
            fs << *ref << "|";
        }
...
std::map<std::string, std::vector<std::string>> setUpReferences(std::string folder,
