...
class GUI_Serv {
    ...
    GUI_Serv(std::string d, Address addr) : httpEndpoint(std::make_shared<Http::Endpoint>(addr));
    dir = d;
    {}
    std::string dir;
    ...
    void setupRouter() {
        ...
        Routes::Get(router, "/users/:id/articles", Routes::bind(&getArts, this));
  ...
  private:
    static void getTopics(const Rest::Request &request, Http::ResponseWriter response) {
        js = "{SEND NUDES}";
        response.send(Http::Code::Ok, js);
    }

    static void getLikes(const Rest::Request &request, Http::ResponseWriter response) {
    ...
    static void getReco(const Rest::Request &request, Http::ResponseWriter response) {
        auto id = request.param(":id").as<std::string>();
        // js = getUserRecs(id)
    ...
    static void getArts(const Rest::Request &request, Http::ResponseWriter response) {
        auto id = request.param(":id").as<std::string>();
        std::vector<std::string> articles = getUserArticles((std::string)id, dir);
    ...
    static void putLikes(const Rest::Request &request, Http::ResponseWriter response) {
        auto id = request.param(":id").as<std::string>();
    ...
    static void postArts(const Rest::Request &request, Http::ResponseWriter response) {
        auto id = request.param(":id").as<std::string>();
        auto bod = request.body();
        std::stringstream ss_body;
        ss_body << bod;
        ...
        bool suc = putUserArticles((std::string)id, out, dir);
        if (suc) {
            response.send(Http::Code::Ok, "{\"success\": 1}");
            return;
        } else {
